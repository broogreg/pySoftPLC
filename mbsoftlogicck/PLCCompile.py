...
class SourceAnalyse:
	...
	def MergeSourceBlock(self, newsubname, ilsource):
		...
		for subname, sublist in list(self._SourceBlocks.items()):
			if (subname != 'main'):
	...
	def _ExtractParams(self, inst):
		...
		paramdict = dict([('inparam%s' % y, x) for x,y in zip(params, list(range(1, len(params) + 1)))])
	...
	def CheckCalls(self):
		...
		for subblock, runglist in list(self._PLCSyntax.items()):
...
class CompileCode:
	...
	def _AssembleILCodeBlocks(self):
		...
		for subsource, runglist in list(self._PLCSyntax.items()):
