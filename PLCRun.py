...
# Note that this code has been simplified to run with pySoftLogic
##############################################################################

from mbsoftlogicck import PLCInterp, DLCkDataTable, DLCkLibs
from softlogic import PLCMemSave, PLCConstants
...
class SoftLogicCK:
	...
	############################################################
	def SetScanRate(self,scanrate=500):
		...
		self._ScanRate = scanrate / 1000.0
	...
	def _ReadInputs(self):
		...
		####################################################
		#  ONLY DOING BOOLEAN INPUTS FOR NOW
		####################################################
		# ReadInputs
		clkbools = []
		boolinputs = {}
		...
		boolvals = ModbusClient(host=PLCConstants.HOST,port=PLCConstants.PORT).read_discrete_inputs(0, bit_nb=255)
		if boolvals != None:
		...
		#for i in self._PLCCurrentIOConfig.IOReadBoolList:
			...
			#boolinputs = dict(list(zip(i['logictable'], boolval)))
			#self._MainInterp.SetBoolData(boolinputs)
		...
		#for i in self._PLCCurrentIOConfig.IOReadWordList:
			...
			## Make a dictionary out of the address keys and values.
			#wordinputs = dict(list(zip(i['logictable'], listval)))
	...
	def _WriteOutputs(self):
		...
		boolvals_dict = self._MainInterp.GetBoolData(clkbools)
		...
		success = ModbusClient(host=PLCConstants.HOST,port=PLCConstants.PORT).write_multiple_coils(0, booloutputs)
		if not success:
