...
function LadderEditor(docref, ladsymbols, rungnumber, matrixdata) {
	...
	this.InstrButtonStorage = ladsymbols.instrbuttonstorage;
	// This is where the address edit fields are stored when we are not using them.
	this.AddressEditStorage = ladsymbols.addresseditstorage;
	...
	this.RungCommentEditField = null;
	...
	this.addressinputs = ladsymbols.addressinputs;
	...
	*/
	function _InitEditRung() {
		...
		// id for rung comment edit text box.
		var editcomments = "rungcomment" + this.RungNumber;
		// id for rung edit buttons div.
		var rungeditbuttonsid = "rungeditbuttons" + this.RungNumber;
		// id for instruction buttons div.
		var runginstrbuttons = "runginstructionbuttons" + this.RungNumber;
		// id for the address editing div.
		var editaddr = "rungaddredit" + this.RungNumber;
		// id for rung comment editing.
		var rungcommenteditname = "rungcommentedit" + this.RungNumber;


		// Add the rung edit buttons. 
		this.RungEditButtons = this.docref.getElementById(rungeditbuttonsid);
		// Done Edit rung button.
		var editbuttondone = this.docref.createElement("button");
		editbuttondone.setAttribute("onclick", "RungDoneEdit('" + this.RungNumber + "');");
		var editbuttonname = this.docref.createTextNode("Done Edit");
		editbuttondone.appendChild(editbuttonname);
		this.RungEditButtons.appendChild(editbuttondone);

		// Cancel edit button.
		var editbuttoncancel = this.docref.createElement("button");
		editbuttoncancel.setAttribute("onclick", "CancelEdit('" + this.RungNumber + "');");
		var editbuttonname = this.docref.createTextNode("Cancel Edit");
		editbuttoncancel.appendChild(editbuttonname);
		this.RungEditButtons.appendChild(editbuttoncancel);

		// Delete rung button.
		var editbuttondelete = this.docref.createElement("button");
		editbuttondelete.setAttribute("onclick", "RungDelete('" + this.RungNumber + "');");
		var editbuttonname = this.docref.createTextNode("Delete Rung");
		editbuttondelete.appendChild(editbuttonname);
		this.RungEditButtons.appendChild(editbuttondelete);

		// Clear rung button.
		var editbuttonclear = this.docref.createElement("button");
		editbuttonclear.setAttribute("onclick", "RungClear('" + this.RungNumber + "');");
		var editbuttonname = this.docref.createTextNode("Clear Rung");
		editbuttonclear.appendChild(editbuttonname);
		this.RungEditButtons.appendChild(editbuttonclear);

		// Comment edit area.
		var commentedit = this.docref.createElement("p");
		var commentedittext = this.docref.createTextNode("Edit Comments: ");
		commentedit.appendChild(commentedittext);
		var commentinput = this.docref.createElement("input");
		commentinput.setAttribute("id", "rungcommentedit" + this.RungNumber);
		commentinput.setAttribute("type", "text");
		commentinput.setAttribute("size", "70");
		commentedit.appendChild(commentinput);
		var commenteditbutton = this.docref.createElement("button");
		commenteditbutton.setAttribute("onclick", "LadEditor.RungCommentEnter();");
		var commentbuttonname = this.docref.createTextNode("Change");
		commenteditbutton.appendChild(commentbuttonname);
		commentedit.appendChild(commenteditbutton);
		this.RungEditButtons.appendChild(commentedit);


		// Rung comment area (for editing).
		this.RungCommentEditField = this.docref.getElementById(editcomments);
		// Fill the edit field with the current comment.
		this.RungCommentEdit = this.docref.getElementById(rungcommenteditname);
		...
		// Show the rung edit and comment edit buttons.
		this.RungEditButtons.setAttribute("class", "rungshow");
		

		// Add the editing buttons to the rung. 
		this.RungInstrButtons = this.docref.getElementById(runginstrbuttons);
		this.RungInstrButtons.setAttribute("class", "rungshow");
		var editbuttons = this.docref.getElementById("buttonssvg");
		this.RungInstrButtons.appendChild(editbuttons);


		// Hide all the address edit fields.
		this.HideAddressEditFields();

		// Add the address editing to the rung.
		this.RungAddrEditing = this.docref.getElementById(editaddr);
		var editaddr = this.docref.getElementById("editaddrprototype");
		this.RungAddrEditing.appendChild(editaddr);
	...
	*/
	function _InitEditMatrix() {
		...
		var editladder = "rungeditladder" + this.RungNumber;
	...
	*/
	function _GenSVGMatrix() {
		...
		var svgcontainer = this.svgcontainer.cloneNode(true);
		svgcontainer.removeAttribute("id");
		var laddercontainer = this.laddercontainer.cloneNode(true);
		laddercontainer.removeAttribute("id");
		...
		// Add the container to the SVG.
		svgcontainer.appendChild(laddercontainer);
	...
	*/
	...
	// ##################################################################
	/* Remove the SVG edit matrix.
	*/
	function _RemoveSVGMatrix() {
		var element = this.RungEditMatrix;
		if (element) {
	...
	this.RemoveSVGMatrix = _RemoveSVGMatrix;
	...
	*/
	function _CloseRung() {
		...
		// Remove the SVG edit matrix
		this.RemoveSVGMatrix();
		// Remove the rung edit buttons.
		this.RemoveRungEditButtons();

		// Hide the address editing.
		this.RungAddrEditing.setAttribute("class", "runghide");
		// Hide the rung instruction buttons.
		this.RungInstrButtons.setAttribute("class", "runghide");
		// Hide the rung edit and comment edit buttons.
		this.RungEditButtons.setAttribute("class", "runghide");

		// Put the rung instruction buttons back where we got them from.
		var instrbuttons = this.docref.getElementById("buttonssvg");
		this.InstrButtonStorage.appendChild(instrbuttons);

		// Put the address edit fields away too.
		var addressedit = this.docref.getElementById("editaddrprototype");
		this.AddressEditStorage.appendChild(addressedit);
	...
	*/
	function _FillMathField(rungdata) {
		...
		this.docref.forms.addreditmath.equation.value = rungdata[1];
		this.docref.forms.addreditmath.oneshot.checked = (rungdata[2] == true);
	...
	*/
	function _GetMathValues() {
		...
		var param2ok = /^[a-zA-Z0-9 \+\-\*\/\^\(\)]+$/.test(equation);


		// Signal if the parameters were ok.
		this.SignalParamStatus("editmathdest", param1ok);
		this.SignalParamStatus("editmathequ", param2ok);
	...
	*/
	function _DisplayRungComment(commenttext) {
		...
		// If there are any existing elements, remove them first.
		if (this.RungCommentEditField.hasChildNodes()) {
			while (this.RungCommentEditField.firstChild) {
				this.RungCommentEditField.removeChild(this.RungCommentEditField.firstChild);
			}
		} 

		// Add the new comments.
		var commentvalue = this.docref.createTextNode(commenttext);
		this.RungCommentEditField.appendChild(commentvalue);
