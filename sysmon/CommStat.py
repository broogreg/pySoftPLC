...
class CommStat:
	...
	def GetCurrentServerReportData(self):
		...
		# List of servers.
		serverlist = [server.GetStatusInfo() for server in ComsConfig.ConfigServer.GetServerList()]
	...
	def GetTCPClientStatusData(self):
		...
		return [client.GetStatusInfo() for client in tcpclientlist]
	...
	def GetGenClientStatusData(self):
		...
		return [client.GetStatusInfo() for client in clientlist]
	...
	def GetCurrentCommsMonitor(self):
		...
		tcpserverstat = [server.GetStatusInfo() for server in serverlist]
		serverstatlist = [{'servername' : server['servername'], 
					'connectioncount': server['connectioncount'], 
					'requestrate' : server['requestrate']} for server in tcpserverstat]
		...
		# TCP client status
		tcpclientlist = ComsConfig.ConfigClient.GetTCPClientList()
		tcpclientstat = [client.GetStatusInfo() for client in tcpclientlist]
		clientstatlist = [{'connectionname' : client['connectionname'],
							'constatus' : client['constatus'],
							'cmdsummary' : client['cmdsummary']} for client in tcpclientstat]
		...
		genclientlist = ComsConfig.ConfigClient.GetGenClientList()
		genclientstat = [client.GetStatusInfo() for client in genclientlist]
		genclientstatlist = [{'connectionname' : client['connectionname'],
							'constatus' : client['constatus'],
							'cmdsummary' : client['cmdsummary']} for client in genclientstat]
	...
	def GetCommsErrorLogs(self, connection):
		...
		# TCP client status
		tcpclientlist = ComsConfig.ConfigClient.GetTCPClientList()
		clientstat = [client.GetStatusInfo() for client in tcpclientlist]
		# Generic client status.
		genclientlist = ComsConfig.ConfigClient.GetGenClientList()
		genclientstat = [client.GetStatusInfo() for client in genclientlist]
		# Combine the lists.
		...
		targetclient = [client for client in clientstat if client['connectionname'] == connection]
		# Did we find it?
