...
class ProgCompiler(PLCCompile.PLCCompiler):
	...
	def CompileAll(self):
		...
		with open(PLCConstants.PLCPRG+".json", 'w') as fout:
			json.dump(self._LadderData, fout)
			
			
		print(self._LadderData)
...
class PComp:
	...
	def _UpdateCompileInfo(self):
		...
		if (not self._SoftLogicCurrentParams['initdone']) or (not self._PLCNewProg.IsCompileError()):
			...
			# A list of subroutines.
			currentprogram['subroutines'] = list(currentprogram['ilblocks'].keys())
	...
	############################################################
	def _CompileNewPLCProg(self, progname):
		...
		self._PLCNewProg = ProgCompiler(progname, timestamp)
	...
	############################################################
	def LoadCompileAndRun(self, program):
		...
		"""
		# Compile.
		cresult = self._CompileNewPLCProg(program)
		if cresult:
			# Update the reporting system with the compile results.
			self._UpdateCompileInfo()
			# Run.
	...
	def AddNewSubr(self, subname):
		...
		formattedtext = ['%s\n' % textline for textline in newsubr.splitlines()]
