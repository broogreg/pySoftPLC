...
class ConfigPLCVal:
	...
	def GetConfigErrorsList(self):
		...
		"""
		configerrors = list(self.GetConfigErrors().values())
	...
	def _VerifyMemSaveAddresses(self, addrlist):
		...
		addrstripped = list(filter(len, addrlist))
		# Find any addresses that are not memory saveable (or may not exist). 
		badaddr = list(itertools.filterfalse(self._MemSaveable, addrstripped))
	...
	def _ConfigAddrUpdate(self, addrconfig):
		...
		for configkeys, configvalues in list(addrconfig.items()):
