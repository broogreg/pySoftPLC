...
class PLCConfig:
	...
	def __init__(self, configname, timestamp):
		...
		self._ValidAddrTypes = list(MBAddrTypes.MaxExtAddrTypes.keys())
	...
	def ReadConfigFile(self):
		...
		try:
			...
			if (filename[0] != self._ConfigFileName):
				...
				return
		except configparser.ParsingError as parserr:
	...
	def PLCNewIOConfig(self, newconfig):
		...
		try:
			sysparams = list(newconfig['sysparams'].items())
			section = '&system'
		...
		for (tag, params) in list(logicioconfig.items()):
			params['logictable'] = ','.join(params['logictable'])

		# Now add these to the parser.
		for (section, params) in list(logicioconfig.items()):
			self._ConfigParser.add_section(section)
			for (option, value) in list(params.items()):
				self._ConfigParser.set(section, option, str(value))
